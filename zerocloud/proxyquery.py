...
from macholib.mach_o import unknown_command
...
class ProxyQueryMiddleware(object):
    ...
    def __init__(self, app, conf, logger=None,
        ...
        self.logger.info('========server restarted with new config===========')
    ...
    def __call__(self, req):
        # self.logger.info("Call is invoked")
        try:
        ...
        if is_zerocloud_request(version, account, req.headers):
            ...
            #self.logger.info("controller found at 686")
            try:
                res = handler(req)
            except HTTPException as error_response:
                self.logger.info("Error:{}".format(error_response))
                return error_response
            self.logger.info("Resp.{}".format(res))
            ...
            #self.logger.info("handling zerocloud request:")
...
class ClusterController(ObjectController):
    ...
    def _get_cluster_config_data_resp(self, req):
        ...
        #self.logger.info("_get_cluster_config invoked at 1406")
        ...
        #self.logger.info("Running on 1436: No issue with Replica")
    ...
    def post_job(self, req):
        ...
        #self.logger.info("Running upto line 1548 inside POST_JOB")
        cluster_config, data_resp = self._get_cluster_config_data_resp(req)
        #self.logger.info("Running upto line 1550 inside POST_JOB")
        if not self.cgi_env:
        ...
        #self.logger.info("Running upto line 1554 inside POST_JOB")
        ...
        load_data_resp = True
        #self.logger.info("Running upto line 1577 inside POST_JOB")
        ...
        # pieces of this.
        #self.logger.info("Running upto line 1590 inside POST_JOB")
        ...
        # for system.maps).
        #self.logger.info("Running upto line 1851 inside POST_JOB")
        ...
        # the object nodes.
        #self.logger.info("Running upto line 1882 inside POST_JOB")
        pile = GreenPileEx(cluster_config.total_count)
        #self.logger.info("Running upto line 1884 inside POST_JOB")
        conns = self._make_exec_requests(pile, exec_requests)
        #self.logger.info("Running upto line 1885 inside POST_JOB")
        ...
        chunked = False
        #self.logger.info("Running upto line 1907 inside POST_JOB")
        try:
        ...
        #self.logger.info("Running upto line 1956 inside POST_JOB")
        ...
        for node in chain(known_nodes, obj_nodes):
            ...
            try:
                ...
                if resp.status == HTTP_CONTINUE:
                    ...
                    #self.logger.info("2358 on Exec")
                    # unknown error
                    # some 500 error that's not insufficient storage
                    self.app.logger.info('Obj server failed with: %d %s'
                                         % (resp.status, resp.reason))
                    conn.error = resp.read()
                    conn.resp = resp
                    resp.nuke_from_orbit()
...
class RestController(ClusterController):
    ...
    @cors_validation
    def POST(self, req):
        self.logger.info("Inside Rest->Post")
        resp = self.handle_request(req)
        if resp:
            return resp
        return HTTPNotImplemented(request=req)
